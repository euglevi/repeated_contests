    ...
    def play_round(self):
        ...
        if (
            ...
            if self.participant.id_in_session in [9, 29, 44, 81]:
                ...
                self.participant.vars["treatment"] == "cost"
                and self.player.id_in_group == self.group.winner1
            ):
                yield pages.BeliefElicitation2, dict(
                    belief2=random.randint(0, self.player.other_endowment2) // 4 * 4
                )
            else:
                yield pages.BeliefElicitation2, dict(
                    belief2=random.randint(0, self.player.other_endowment2)
                )
        else:
            pass
        ...
        if (
            ...
            if (
                ...
                yield pages.Stage2, dict(
                    tokens2=random.randint(0, self.player.endowment2) // 4 * 4
                )
            else:
                yield pages.Stage2, dict(
                    tokens2=random.randint(0, self.player.endowment2)
                )
        else:
            pass

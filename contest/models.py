...
class Subsession(BaseSubsession):
    ...
    def group_by_arrival_time_method(self, waiting_players):
        baseline = [
            p for p in waiting_players if p.participant.vars["treatment"] == "baseline"
        ]
        productive = [
            p
        ...
        cost = [p for p in waiting_players if p.participant.vars["treatment"] == "cost"]
        budget = [
            p for p in waiting_players if p.participant.vars["treatment"] == "budget"
        ]
    ...
    def set_earnings1(self):
        if self.expiry_group is False:
            ...
            if self.total_tickets1 != 0:
                self.winner1 = random.choices(
                    [p.id_in_group for p in self.get_players()],
                    weights=[p.tokens1 for p in self.get_players()],
                )[0]
                for p in self.get_players():
                    p.earnings1 = (
                        p.endowment1 - p.tokens1 + Constants.prize
                        if p.id_in_group is self.winner1
                        else p.endowment1 - p.tokens1
                    )
                    if p.treatment == "budget":
                        p.endowment2 = 25 if p.id_in_group is self.winner1 else 16
            else:
                for p in self.get_players():
            ...
            for p in self.get_players():
                if p.treatment in ["baseline", "budget"]:
                    ...
                    p.tickets2 = (
                        p.tokens2
                        if p.id_in_group is self.winner1
                        else int(p.tokens2 / Constants.ratio_costprod)
                    )
            self.total_tickets2 = sum([p.tickets2 for p in self.get_players()])
            if self.total_tickets2 != 0:
                self.winner2 = random.choices(
                    [p.id_in_group for p in self.get_players()],
                    weights=[p.tickets2 for p in self.get_players()],
                )[0]
                for p in self.get_players():
                    p.earnings2 = (
                        p.endowment2 - p.tokens2 + Constants.prize
                        if p.id_in_group is self.winner2
                        else p.endowment2 - p.tokens2
                    )
            else:
                for p in self.get_players():
                    p.earnings2 = p.endowment2 - p.tokens2
...
class Player(BasePlayer):
    ...
    tokens1 = models.IntegerField(
        label="How many tokens would you like to use \
                                  to buy lottery tickets in Stage 1?",
        min=0,
    )
    tokens2 = models.IntegerField(
        label="How many tokens would you like to use \
                                  to buy lottery tickets in Stage 2?",
        min=0,
    )
    ...
    other_endowment2 = models.IntegerField()
    ...
    def tokens2_error_message(self, value):
        if (
            self.treatment == "cost"
            and value % Constants.ratio_costprod != 0
            and self.id_in_group is not self.group.winner1
        ):
            return "The number of tokens must be a multiple of 4, i.e. \
                the price of each ticket for you"

    def record_other_earnings1(self):
        self.other_earnings1 = [p.earnings1 for p in self.get_others_in_group()][0]

    def record_other_endowment2(self):
        self.other_endowment2 = [p.endowment2 for p in self.get_others_in_group()][0]
    ...
    belief2 = models.IntegerField(
        label="In your estimate, how many tokens will your"
    ...
    def belief2_max(self):
        return self.other_endowment2

    def belief2_error_message(self, value):
        if (
            self.treatment == "cost"
            and value % Constants.ratio_costprod != 0
            and self.id_in_group is self.group.winner1
        ):
            return "The number of tokens must be a multiple of 4, i.e. \
    ...
    def set_earnings_beliefs(self):
        ...
        if self.participant.vars["belief1"] in range(
            self.group.total_tickets1 - self.tokens1 - 1,
            self.group.total_tickets1 - self.tokens1 + 2,
        ):
            self.bonus1 = Constants.bonus
        else:
            self.bonus1 = 0
        if self.belief2 in range(self.other_tokens2 - 1, self.other_tokens2 + 2):
